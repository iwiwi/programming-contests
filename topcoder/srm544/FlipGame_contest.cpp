...
int FlipGame::minOperations(vector <string> board) {
  ...
  for (int x = W - 1; x >= 0; --x) {
    vector<int> nxt(H, 0);
    rep (y, H - 1) if (board[y][x] != board[y + 1][x]) nxt[y + 1] = 1;
    nxt[0] = (accumulate(all(nxt), 0) + (board[H - 1][x] - '0')) % 2;

    // printf("x = %2d: ", x); rep (y, H) printf("%d ", nxt[y]); puts("");

    rep (y, H) if (nxt[y]) {
      int ty;
      for (ty = y; ty < H; ++ty) if (crr[ty] > 0) break;
      if (ty < H) --crr[ty];
    }

    int rest = accumulate(all(crr), 0);
    bool odd = rest % 2;
    ...
    crr.swap(nxt);
    // printf("%d\n", crr[0]);
    // printf("x = %2d: ", x); rep (y, H) printf("%d ", crr[y]); puts("");
  }

  return accumulate(all(crr), 0);
}
