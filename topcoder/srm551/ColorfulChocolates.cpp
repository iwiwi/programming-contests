...
int ColorfulChocolates::maximumSpread(string chocolates, int maxSwaps) {
  ...
  rep (i, N) {
    ...
    int j;
    for (j = i + 1; j < N; ++j) if (chocolates[j] != c) break;

    vector<int> v;
    {
      int t = 1;
      for (int k = i - 1; k >= 0; --k) {
        if (chocolates[k] == c) v.pb(i - k - t++);
      }
    }
    {
      int t = 0;
      for (int k = j; k < N; ++k) {
        if (chocolates[k] == c) v.pb(k - j - t++);
      }
    }
    sort(all(v));
    // printf("[%d, %d):", i, j); rep (k, v.size()) printf(" %d", v[k]); puts("");

    int s = maxSwaps, a = j - i;
    rep (i, v.size()) {
      if (s < v[i]) break;
      s -= v[i];
      ++a;
