...
string TheLargestString::find(string s, string t) {
  ...
  string sma[60];
  rep (i, L) for (int k = L; k > 0; --k) {
    sma[k] = max(sma[k], sma[k - 1] + s[i]);
  }
  // rep (i, L + 1) cout << sma[i] << endl;
  ...
  for (int l = 1; l <= L; ++l) {
    ...
    string tma[60];
    rep (i, L) for (int k = l; k > 0; --k) {
      if (s[i] == ss[k - 1] && (int)tma[k - 1].length() == k - 1) {
        tma[k] = max(tma[k], tma[k - 1] + t[i]);
      }
    }
    // cout << ss << " " << tma[l] << endl;
    // assert(tma[l].length() == l);
    ans = max(ans, ss + tma[l]);
  }
