...
string GogoXCake::solve(vector <string> cake, vector <string> cutter) {
  int W = cake[0].length(), H = cake.size();
  int w = cutter[0].length(), h = cutter.size();

  for (int y = 0; y + h <= H; ++y) {
    for (int x = 0; x + w <= W; ++x) {
      rep (dy, h) rep (dx, w) {
        if (cutter[dy][dx] == '.' && cake[y + dy][x + dx] != '.') goto ng;
      }

      rep (dy, h) rep (dx, w) {
        if (cutter[dy][dx] == '.') cake[y + dy][x + dx] = 'X';
      }
  ...
  rep (y, H) rep (x, W) if (cake[y][x] == '.') return "NO";
  return "YES";
}
