...
void naive_search(string &k, int p, string &ans) {
  if (p == (int)k.length()) {
    vector<int> perm(M);
    rep (i, M) perm[i] = i;
    do {
      rep (i, M) {
        string s1 = k.substr(i * L, L), s2 = K2.substr(perm[i] * L, L);
        rep (j, L) {
          if (s1[j] == s2[j]);
          else if (s1[j] == '?' || s2[j] == '?');
          else goto dmp;
   ...
   dmp:;
    ...
    if (k[p] != '?') naive_search(k, p + 1, ans);
    else {
      for (char c = 'a'; c <= 'f'; ++c) {
        k[p] = c;
        naive_search(k, p + 1, ans);
      }
      k[p] = '?';
    }
  }
...
string naive() {
  string tmp = K1, ans(K1.length(), 'z');
  naive_search(tmp, 0, ans);
...
bool test(string K) {
  ...
  rep (v, V) adj[v].clear();
  rep (i, M) rep (j, M) {
    string s1 = K.substr(i * L, L), s2 = K2.substr(j * L, L);
    rep (k, L) {
      if (s1[k] == s2[k]);
      else if (s1[k] == '?' || s2[k] == '?');
      else goto dmp;
...
void solve() {
  ...
  if (!test(K)) K = "IMPOSSIBLE";
  else {
    rep (i, K.length()) if (K[i] == '?') {
      for (char c = 'a'; c <= 'f'; ++c) {
        K[i] = c;
        if (test(K)) goto done;
      }
