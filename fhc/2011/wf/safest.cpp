...
int score(int x, int y, int z) {
  if (x < 0 || 1000 < x ||
      y < 0 || 1000 < y ||
      z < 0 || 1000 < z) return INT_MIN;

  int r = INT_MAX;
  rep (i, N) {
    int t = sqr(x - X[i]) + sqr(y - Y[i]) + sqr(z - Z[i]);
    r = min(t, r);
  }
  return r;
}

pair<int, pair<int, pair<int, int> > > pts[T];
...
void solve() {
  rep (t, T) {
    ...
    int y = rand() % 1001;
    int z = rand() % 1001;
    pts[t] = mp(score(x, y, z), mp(x, mp(y, z)));
  }
    ...
    for (int dx = -W; dx <= W; ++dx) {
      for (int dy = -W; dy <= W; ++dy) {
        for (int dz = -W; dz <= W; ++dz) {
          int x = pts[t].second.first + dx;
          int y = pts[t].second.second.first  + dy;
          int z = pts[t].second.second.second + dz;
          ans = max(ans, score(x, y, z));
        }
      }
    }
  }
