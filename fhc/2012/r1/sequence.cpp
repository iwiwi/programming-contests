...
vector<int> merge(vector<int> arr1, vector<int> arr2) {
  ...
  for (; i1 < arr1.size(); ++i1) result.push_back(arr1[i1]);
  for (; i2 < arr2.size(); ++i2) result.push_back(arr2[i2]);
  return result;
}
...
vector<int> merge_sort(vector<int> arr) {
  ...
  vector<int> first_half  = merge_sort(vector<int>(arr.begin(), arr.begin() + mid));
  vector<int> second_half = merge_sort(vector<int>(arr.begin() + mid, arr.end()));
...
void solve() {
  ...
  ll result = 1;
  rep (i, N) result = (31 * result + ans[i]) % 1000003;
  printf("%lld\n", result);
}
