...
struct player_t {
  ...
  int total_time_played, rank;
  bool playing;
};

int N, M, P;
vector<player_t> all_players;

void input() {
  cin >> N >> M >> P;
  all_players.resize(N);
  rep (i, N) {
    cin >> all_players[i].name >> all_players[i].shot_percentage >> all_players[i].height;
    all_players[i].total_time_played = 0;
  }
}

void solve() {
  sort(all(all_players),
       [&](const player_t &a, const player_t &b) {
         return mp(a.shot_percentage, a.height) > mp(b.shot_percentage, b.height);
       });
  rep (i, N) all_players[i].rank = i;

  vector<string> ans;
  rep (iter, 2) {
    vector<player_t> team_players;
    rep (i, N) if (all_players[i].rank % 2 == iter) {
      team_players.pb(all_players[i]);
    }

    // Time 0
    rep (i, team_players.size()) team_players[i].playing = i < P;

    rep (m, M) {  // Time |m|
      // Play
      rep (i, team_players.size()) {
        if (team_players[i].playing) ++team_players[i].total_time_played;
      }

      // Rotate
      if ((int)team_players.size() > P) {
        auto f = [&](const player_t &a, const player_t &b) {
          return
          make_tuple(a.playing, a.total_time_played, a.rank) <
          make_tuple(b.playing, b.total_time_played, b.rank);
        };
        auto in  = min_element(all(team_players), f);
        auto out = max_element(all(team_players), f);
        assert(in ->playing == false);
        assert(out->playing == true );
        in ->playing = true;
        out->playing = false;
        // cerr << in->name << " -> " << out->name << endl;
      }

      assert(count_if(all(team_players), [&](const player_t &a) { return a.playing; }) == P);
    }

    rep (i, team_players.size()) {
      if (team_players[i].playing) ans.pb(team_players[i].name);
    }
  }

  sort(all(ans));
  assert((int)ans.size() == 2 * P);
  rep (i, ans.size()) {
