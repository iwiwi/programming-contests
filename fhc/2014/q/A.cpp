...
void solve() {
  int xlb = INT_MAX, xub = INT_MIN, ylb = INT_MAX, yub = INT_MIN;
  rep (y, N) rep (x, N) if (F[y][x] == '#') {
    xlb = min(xlb, x); xub = max(xub, x);
    ylb = min(ylb, y); yub = max(yub, y);
  }
  assert(xlb <= xub && ylb <= yub);

  if (xub - xlb != yub - ylb) goto dmp;
