...
const int dx[] = { 0, +1,  0, -1,  0, +1};
const int dy[] = {+1,  0, -1,  0, +1,  0};
...
int search(int x, int y, int d) {
  if (x < 0 || W <= x || y < 0 || H <= y || F[y][x] == '#') return 0;
  if (mem[y][x][d] != -1) return mem[y][x][d];

  int ans = 1;
  for (int td : nxt[d]) ans = max(ans, 1 + search(x + dx[td], y + dy[td], td));
  return mem[y][x][d] = ans;
}
