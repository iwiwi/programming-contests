...
void gen() {
  ...
  for (ll i = 1; i < max(N, M); ++i) {
    if (i < N) C[0][i] = (a1 * C[0][(i-1) % N] + b1 * C[1][(i-1) % M] + c1) % r1;
    if (i < M) C[1][i] = (a2 * C[0][(i-1) % N] + b2 * C[1][(i-1) % M] + c2) % r2;
  }
}

ll next_color(ll x, ll *cs, ll n, map<ll, ll> &me, map<ll, ll> &op) {
  {
    ll c = cs[x];
    assert(me[c] == 0);
    ++me[c];
...
void solve() {
  memset(C, -1, sizeof(C));
  gen();
  rep (i, N) assert(C[0][i] != -1);
  rep (i, M) assert(C[1][i] != -1);

  dif = 0;
  ll x = 0, y = 0, ans = 0;
  while (x < N && y < M) {
    ll tx = next_color(x, C[0], N, mem[0], mem[1]);
    ll ty = next_color(y, C[1], M, mem[1], mem[0]);
    if (dif == 0) ans += (tx - x) * (ty - y);
    x = tx;
    y = ty;
  }
  printf("%lld\n", ans);
