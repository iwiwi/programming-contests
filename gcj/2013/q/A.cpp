...
bool does_win(char c) {
  rep (y, N) {
    rep (x, N) if (fld[y][x] != c && fld[y][x] != 'T') goto dmp1;
    return true;
 dmp1:;
  }
  rep (x, N) {
    rep (y, N) if (fld[y][x] != c && fld[y][x] != 'T') goto dmp2;
    return true;
 dmp2:;
  }
  {
    rep (i, N) if (fld[i][i] != c && fld[i][i] != 'T') goto dmp3;
    return true;
 dmp3:;
  }
  {
    rep (i, N) if (fld[i][N - i - 1] != c && fld[i][N - i - 1] != 'T') goto dmp4;
    return true;
 dmp4:;
  }
  return false;
...
void solve() {
  if (does_win('X')) puts("X won");
  else if (does_win('O')) puts("O won");
  else {
    rep (y, N) rep (x, N) if (fld[y][x] == '.') goto incomplete;
    puts("Draw");
    return;
 incomplete:
