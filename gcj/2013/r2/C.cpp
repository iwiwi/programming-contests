...
int eval(int i) {
  ...
  dfs(i);
  vector<int> re;
  rep (k, N) if (vis[k] && i != k) re.pb(k);
  sort(all(re));

  // debug("%d: ", i); rep (j, re.size()) debug("%d", re[j]); debug("\n");

  int t = 1;
  rep (j, re.size()) t = max(t, eval(re[j]) + 1);
  while (usd[t]) ++t;
...
void solve() {
  ...
  // rep (i, N) { debug("%d: ", i); rep (j, adj[i].size()) debug("%d ", adj[i][j]); debug("\n"); }

  memset(mem, -1, sizeof(mem));
  memset(usd, 0, sizeof(usd));
  rep (i, N) printf("%d%c", eval(i), i == N - 1 ? '\n' : ' ');
}
