...
void input() {
  ...
  N = 1 << P;
  rep (i, N) scanf("%d", &M[i]);
  rep (i, P) rep (j, (1 << (P - i - 1))) scanf("%d", &C[i][j]);
}
...
int mem[15][1100][1100];

int saiki(int d, int k, int m) {
  if (d == -1) {
    if (m > M[k]) return INF;
    else return 0;
  }

  if (mem[d][k][m] != -1) return mem[d][k][m];

  int res = INF;
  res = min(res, saiki(d - 1, k * 2, m + 1) + saiki(d - 1, k * 2 + 1, m + 1));
  res = min(res, saiki(d - 1, k * 2, m)     + saiki(d - 1, k * 2 + 1, m)   + C[d][k]);

  // printf("%d, %d, %d: %d (%d)\n", d, k, m, res, C[d][k]);
  return mem[d][k][m] = res;
}
