...
void solve() {
  ...
  for (int c = 0; c < N; ++c) {
    for (int s = -1; s <= 1; s += 2) {
      ...
      rep (i, M) rep (j, M) {
        int tj = 2 * x - j;
        if (tj < 0 || M <= tj) continue;
        if (fld[i][j] != -1 && fld[i][tj] != -1 && fld[i][j] != fld[i][tj]) {
          // printf("  ng: %d, %d, %d\n", i, j, tj);
          goto ngx;
        }
      }
    ...
    for (int s = -1; s <= 1; s += 2) {
      ...
      rep (i, M) rep (j, M) {
        int ti = 2 * y - i;
        if (ti < 0 || M <= ti) continue;
        if (fld[i][j] != -1 && fld[ti][j] != -1 && fld[i][j] != fld[ti][j]) goto ngy;
      }
