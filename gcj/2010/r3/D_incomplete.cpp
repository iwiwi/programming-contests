...
void solve() {
  ...
  for (int i = 0; i <= B; ++i) {
    for (int s = 0; s <= S; ++s) debug("%lld ", dp2[i][s]);
    debug("\n");
  }
    ...
    for (int c = 0; c < B; ++c) {
      for (int r = 0; r <= B; ++r) {
        for (int tc = 0; tc < B; ++tc) {
          ...
          int sc = tc * B + dig[p] - c;
          if (sc < 0 || S < sc) continue;
          // ll tmp = 0;

          for (int tr = 0; tr <= r; ++tr) {
            //tmp += dp2[tr][sc];
            // tmp %= MOD;
            // dp[p + 1][tc][tr] += tmp * dp[p][c][r];
            // dp[p + 1][tc][tr] %= MOD;
            
            dp[p + 1][tc][tr] += dp2[tr][sc] * dp[p][c][r];
            dp[p + 1][tc][tr] %= MOD;
          }
        }
      }
