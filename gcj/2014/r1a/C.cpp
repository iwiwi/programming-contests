...
double eval(const vector<int> a) {
  double t = 0;
  rep (j, a.size()) t += cnt[a[j]][j];
  return t;
}
...
void init() {
  ...
  avgA = accumulate(all(A), 0.0) / A.size();
  avgB = accumulate(all(B), 0.0) / B.size();
  fprintf(stderr, "%.10f\n", avgA);
  fprintf(stderr, "%.10f\n", avgB);
}
...
void solve() {
  double e = eval(A);
  debug("%.10f\n", e);
  if (fabs(e - avgA) < fabs(e - avgB)) puts("BAD");
  else puts("GOOD");
}
