...
void solve() {
  ...
  rep (y, H) rep (x, W) {
    if (!fld[y][x]) add_edge(vid(x, y, 0), vid(x, y, 1), 1);
  }
  rep (x, W) add_edge(S, vid(x, 0, 0), 1);
  rep (x, W) add_edge(vid(x, H - 1, 1), T, 1);
  ...
  rep (y, H) rep (x, W) rep (d, 4) {
    ...
    add_edge(vid(x, y, 1), vid(tx, ty, 0), 1);
  }
